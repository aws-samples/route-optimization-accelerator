...
import { listFleetHandler } from "@route-optimization-accelerator/web-api-service-typescript-runtime";
import {
  DEFAULT_PAGE_SIZE,
  getDynamoDBClient,
...
export const handler = listFleetHandler(
  customHeadersInterceptor,
  async ({ input }) => {
    try {
      ...
      const results = await ddb.send(
        new QueryCommand({
          ...
          IndexName: process.env.FLEET_ACTIVE_SORTED_INDEX,
    ...
    return buildInternalServerError("Unable to retrieve the fleet members");
  },
);
