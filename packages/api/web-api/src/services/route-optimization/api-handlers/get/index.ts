...
export const handler = getRouteOptimizationHandler(
  customHeadersInterceptor,
  async ({ input }) => {
    ...
    try {
      ...
      return {
        statusCode: 200,
        body: {
          data: result.Item as Optimization,
        },
      };
    } catch (ex) {
      console.error("Error retrieving the item from the database");
      console.error(ex);
    }

    return buildInternalServerError("Unable to retrieve the optimization");
  },
);
