...
import { listRouteOptimizationHandler } from "@route-optimization-accelerator/web-api-service-typescript-runtime";
import {
  DEFAULT_PAGE_SIZE,
  getDynamoDBClient,
...
export const handler = listRouteOptimizationHandler(
  customHeadersInterceptor,
  async ({ input }) => {
    try {
      ...
      const results = await ddb.send(
        new QueryCommand({
          ...
          IndexName: process.env.OPTIMIZATION_TASK_ACTIVE_SORTED_INDEX,
          // use problem id since this entity doesn't have a name
          ...
          ExpressionAttributeNames: {
            "#isActive": "isActive",
            ...(hasNameFilter ? { "#problemId": "problemId" } : {}),
          },
          ExpressionAttributeValues: {
            ":isActive": "Y",
            ...(hasNameFilter ? { ":problemId": name } : {}),
          },
    ...
    return buildInternalServerError("Unable to retrieve the optimizer list");
  },
);
