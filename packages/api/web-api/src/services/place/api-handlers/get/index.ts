...
export const handler = getPlaceHandler(
  customHeadersInterceptor,
  async ({ input }) => {
    try {
      ...
      return {
        statusCode: 200,
        body: {
          data: result.Item as Place,
        },
      };
    } catch (err) {
      console.error("Error retrieving the place");
      console.error(err);
    }

    return buildInternalServerError("Unable to retrieve the Place");
  },
);
