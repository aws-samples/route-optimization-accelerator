...
import { listOrdersHandler } from "@route-optimization-accelerator/web-api-service-typescript-runtime";
import {
  DEFAULT_PAGE_SIZE,
  getDynamoDBClient,
...
export const handler = listOrdersHandler(
  customHeadersInterceptor,
  async ({ input }) => {
    try {
      ...
      const results = await ddb.send(
        new QueryCommand({
          ...
          IndexName: process.env.ORDER_ACTIVE_SORTED_INDEX,
          ...
          ExpressionAttributeNames: {
            "#isActive": "isActive",
            ...(hasNameFilter ? { "#number": "number" } : {}),
          },
          ExpressionAttributeValues: {
            ":isActive": "Y",
            ...(hasNameFilter ? { ":number": name } : {}),
          },
    ...
    return buildInternalServerError("Unable to retrieve the order list");
  },
);
