...
export interface FleetFinderProps {
  selectedItemValue?: string;
  onSelect?: (option: Optional<Fleet>) => void;
}

export const FleetFinder: React.FC<FleetFinderProps> = ({
  selectedItemValue,
  onSelect,
}) => {
  ...
  const [rawData, setRawData] = useState<Optional<{ [key: string]: Fleet }>>();
  ...
  const fetch = async () => {
    try {
      ...
      const response = await webApi.listFleet({
        name: debouncedValue,
        limit: 50_000,
      });
